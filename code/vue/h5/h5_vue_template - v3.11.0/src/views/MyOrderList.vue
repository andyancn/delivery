...
<script>
  ...
  import { orderList } from '../api/public';
  ...
  export default {
    ...
    methods: {
      ...
      selectOrderStatus(e) {
        ...
        thiz.orderList = [];
        thiz.pageIndex = 1;
        thiz.getOrderList(thiz.orderType, e);
      },
      getOrderList(orderType, orderStatus) {
        let thiz = this;
        let req = {
          page: thiz.pageIndex,
        ...
        orderList(req).then(res => {
          if (res.data.data.itemList && res.data.data.itemList.length > 0) {
            ...
            for (let one of tmpList) {
              ...
              one.statusName = Dict.getOrderStatusName(one.orderStatus);
            }
      ...
      onLoad() {
        ...
        thiz.getOrderList(thiz.orderType, thiz.orderStatus);
      },
